...
import org.omg.schema.spec.kdm._1_1.code.CodeModel;
import org.omg.schema.spec.kdm._1_1.code.MethodKind;
import org.omg.schema.spec.kdm._1_1.code.MethodUnit;
import org.omg.schema.spec.kdm._1_1.code.Package;
import org.omg.schema.spec.kdm._1_1.code.StorableKind;
import org.omg.schema.spec.kdm._1_1.code.StorableUnit;
import org.omg.schema.spec.kdm._1_1.kdm.KDMFramework;
import org.omg.schema.spec.kdm._1_1.kdm.Segment;
...
public class KDMImporter_v1_1 implements IStaticsImporter {

	private static final java.util.logging.Logger logger = java.util.logging.Logger
			.getLogger(KDMImporter_v1_1.class.getName());
	private final IFeatureModel featureModel;
	private final StaticsFactory factory;

	/**
	 * Constructs a new instance of {@link KDMImporter_v1_1}.
	 * 
	 * @param featureModel
	 */
	public KDMImporter_v1_1(IFeatureModel featureModel) {
	...
	private List<DPackage> convertKDMToFeatureElements(Segment segment) {
		...
		for (KDMFramework model : segment.getSegmentOrModel()) {
		...
		for (AbstractCodeElement element : codeModel.getCodeElement()) {
			if (element instanceof Package) {
				Package pa = (Package) element;
				// only add packages containing packages and/or classes
				if (pa.getCodeElement().size() > 0) {
	...
	private DPackage addPackage(InventoryModel inventoryModel, Package element) {
		String path = KDMHelper_v1_1.getPackagePath(inventoryModel, element);
	...
	private void addClass(DPackage parent, ClassUnit element, InventoryModel inventoryModel) {
		...
		SourceRegion sourceRegion = KDMHelper_v1_1.getSourceRegion(element);
		SourceFile invItem = (SourceFile) KDMHelper_v1_1.getCorrespondingInventoryElement(sourceRegion
	...
	private void addAttributesAndMethods(DClass parent, ClassUnit element) {
		List<CodeItem> items = element.getCodeElement();
	...
	private void addClasses(DPackage pkg, Package element, InventoryModel inventoryModel) {
		List<AbstractCodeElement> classes = element.getCodeElement();
